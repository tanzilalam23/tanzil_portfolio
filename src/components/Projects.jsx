...
const projectsMeta = [
  ...
  },
  {
    ...
    links: [{ label: 'GitHub', url: 'https://github.com/tanzilalam23/Detecting-Data-Drift-_Automated' }],
  },
  {
    tech: ['Python', 'Classifier', 'Confusion Matrix', 'sklearn', 'metrics score'],
    links: [{ label: 'GitHub', url: 'https://github.com/tanzilalam23/KNN-ML-project' }],
  },
  {
    tech: ['Python', 'sklearn', 'pandas', 'numpy', 'matplotlib'],
    links: [{ label: 'GitHub', url: 'https://github.com/tanzilalam23/NASA-Battery-project' }],
  },
  {
    tech: ['Feature Scaling', 'K-Neighbors Classifier', 'Python', 'sklearn', 'StandardScaler', 'seaborn', 'matplotlib'],
    links: [{ label: 'GitHub', url: 'https://github.com/tanzilalam23/Drug-Classification' }],
  },
  {
    tech: ['Gemini LLM', 'Gen AI', 'StandardScaler', 'streamlit', 'pdf2image'],
    links: [{ label: 'GitHub', url: 'https://github.com/tanzilalam23/ATS' }],
  },
  {
...
const ProjectCard = ({ project, index, hoverHint }) => {
      ...
      >
        ...
        <div className="absolute inset-0 w-full h-full backface-hidden rounded-2xl 
          ...
          <div className="h-full flex flex-col">
            <h4 className="text-xl font-bold text-cyan-400 mb-3 leading-tight">{project.title}</h4>
            ...
            <div className="flex flex-wrap gap-2">
              {project.tech.slice(0, 9).map((tech, i) => (
                <span key={i} className="text-xs px-3 py-1 rounded-full bg-cyan-400/20 text-cyan-300 border border-cyan-400/30 font-medium">
                  {tech}
                </span>
              ))}
              {project.tech.length > 8 && (
                <span className="text-xs px-3 py-1 rounded-full bg-gray-700/50 text-gray-400 border border-gray-600/50 font-medium">
        ...
        <div className="absolute inset-0 w-full h-full backface-hidden rounded-2xl rotate-y-180
          ...
          <h4 className="text-lg font-bold text-cyan-400 text-center mb-2">{project.title}</h4>
          ...
          <div className="text-xs text-gray-400 text-center mt-2">{hoverHint}</div>
        </div>
      </div>
    </div>
  );
...
const Projects = () => {
  ...
  const projects = t.projects.items.map((item, i) => ({
    ...item,
    tech: projectsMeta[i].tech,
    links: projectsMeta[i].links,
  }));
          ...
          <div className="container mx-auto px-6 md:px-16 max-w-7xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {projects.map((project, index) => (
                ...
                >
                  <ProjectCard project={project} index={index} hoverHint={t.projects.hoverHint} />
                </div>
              ))}
            </div>
