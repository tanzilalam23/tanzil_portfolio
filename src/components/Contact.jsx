  ...
  return (
    <div id="contactOptions" className="relative py-12 sm:py-16 bg-gray-900 text-center min-h-screen flex flex-col justify-center px-4">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/4 left-1/4 w-32 sm:w-64 h-32 sm:h-64 opacity-20 rounded-full blur-3xl"
          style={{ backgroundColor: 'rgb(0,255,186)' }} />
        <div className="absolute bottom-1/4 right-1/4 w-32 sm:w-64 h-32 sm:h-64 opacity-15 rounded-full blur-3xl"
      ...
      <div className="relative z-10">
            ...
            return (
              <button
                ...
                style={{
                  ...
                  boxShadow: isPressed
                    ? 'inset 0 4px 12px rgba(0,0,0,0.4), inset 0 -2px 8px rgba(0,255,186,0.1), 0 2px 8px rgba(0,0,0,0.3)'
                    : '0 8px 20px rgba(0,0,0,0.3), 0 4px 12px rgba(37,40,131,0.4), inset 0 1px 0 rgba(0,255,186,0.2), inset 0 -1px 0 rgba(0,0,0,0.2)',
                }}
              >
                <div
                ...
                <div
                  ...
                  style={{ background: 'radial-gradient(circle, rgba(0,255,186,0.3) 0%, transparent 70%)' }}
                />
                <div className="relative z-10 flex flex-col items-center justify-center h-full p-3 sm:p-4">
                  <div className="mb-2 sm:mb-3 drop-shadow-sm transition-transform duration-200 group-hover:scale-110"
                    style={{ color: 'rgb(0,255,186)' }}>
                    {option.icon}
                  </div>
                  <div className="font-semibold text-sm sm:text-base" style={{ color: 'rgb(0,255,186)' }}>
                    {option.label}
                  </div>
                </div>
                <div className="absolute top-2 left-2 w-2 h-2 rounded-full opacity-20" style={{ backgroundColor: 'rgb(0,255,186)' }} />
                <div className="absolute bottom-2 right-2 w-1 h-1 rounded-full opacity-10" style={{ backgroundColor: 'rgb(0,255,186)' }} />
              </button>
            );
          })}
        </div>
        ...
        <div className="min-h-[100px] flex items-center justify-center px-4">
          {activeKey && (
            <div key={activeKey} className="animate-fade-in text-center w-full max-w-md">
              {contactOptions
                .filter((opt) => opt.id === activeKey && opt.type === 'info')
                .map((opt) => (
                  <button
                    key={opt.id}
                    className="p-4 sm:p-6 rounded-2xl backdrop-blur-sm border cursor-pointer hover:scale-105 transition-all duration-300 w-full group"
                  ...
                  >
                    <p className="font-medium text-lg sm:text-2xl mb-2 break-all" style={{ color: 'rgb(0,255,186)' }}>
                      {opt.value}
                    </p>
                    <p className="text-gray-300 text-xs sm:text-sm mb-2">📋 Click to copy to clipboard</p>
                    {copied && (
